...
from functions.NGCM_scheme import NGCM_restoration
...
class Diffusion(object):
    ...
    def sample(self, logger):
        ...
        self.NGCM_wrapper(model, logger)

    def NGCM_wrapper(self, model, logger):
        ...
        for x_orig, classes in pbar:
            ...
            with torch.no_grad():
                x = NGCM_restoration(
                    ...
                    mu_factor=getattr(args, 'mu_factor', 1.0),
                    use_dsr=bool(getattr(args, 'use_dsr', 0)),
                    use_nnp=bool(getattr(args, 'use_nnp', 0)),
                    nnp_steps=getattr(args, 'nnp_steps', ''),
                    dsr_repeat=int(getattr(args, 'dsr_repeat', 1)),
                    dsr_repeat_damp=float(getattr(args, 'dsr_repeat_damp', 1.0)),
                    dsr_repeat_noise_damp=float(getattr(args, 'dsr_repeat_noise_damp', 1.0)),
                    save_root=self.args.image_folder,  
                    x_orig=x_orig,                    
